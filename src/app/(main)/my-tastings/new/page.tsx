...
interface NewTastingPageProps {
  searchParams: { whisky?: string };
}

export default async function NewTastingPage({ searchParams }: NewTastingPageProps) {
  const session = await getSession();
  if (!session) redirect("/sign-in?return=/my-tastings/new");

  // Viski seçilmeden not yazılamaz — katalogdan seçime yönlendir
  if (!searchParams.whisky) redirect("/whiskeys");

  await connectToDatabase();
  const whiskey = await whiskeyService.findWhiskeyBySlug(searchParams.whisky);
  if (!whiskey) redirect("/whiskeys");
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 px-4 py-10 sm:px-6">
      <Button asChild variant="ghost" size="sm">
        <Link href={`/whiskeys/${whiskey.slug}`}>
