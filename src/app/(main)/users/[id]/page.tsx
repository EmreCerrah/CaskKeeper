...
export default async function PublicProfilePage({ params, searchParams }: ProfilePageProps) {
  ...
  const stats = [
    ...
    { label: t("profile.statFollowers"), value: profile.followerCount, href: `/users/${profile.id}/followers` },
    { label: t("profile.statFollowing"), value: profile.followingCount, href: `/users/${profile.id}/following` },
  ];
